...
internal class TokenRangeManager
{
    ...
    private readonly string _sqlQuery =
        $$"""
               ...
               VALUES (
                   @MachineIdentifier,
                   COALESCE((SELECT MAX("End") FROM "TokenRanges") + 1, {{DefaultPageSize}}),
                   COALESCE((SELECT MAX("End") FROM "TokenRanges") + {{DefaultPageSize}}, 2000)
               )
    ...
    public async Task<TokenRangeResponse> AssignRangeAsync(string machineIdentifier)
    {
        ...
        throw new Exception("Failed to assign range.");
    }
}